...
| # | Item | Status | Notes |
|---|------|--------|-------|
| TV1 | **`vizora-tv` unit tests fail on Node 24 but pass in CI** | OPEN — re-scoped 2026-08-11 | **Not product debt — the earlier "5 pre-existing failures" note was wrong.** CI runs all 296 on the same SHA: **294 pass, 2 skip, 0 fail**. The five fail only on local Node 24. CI pins `node-version: 20`, the suite runs in vitest's `node` environment, and **Node 21+ made `navigator` a built-in global** — `vizora-app.spec.ts:200` does `vi.stubGlobal('navigator', …)` and `platform.ts:73` reads `navigator.userAgent` for platform detection, so on Node 24 the stub no longer shadows cleanly and `update_config` rejects origins the tests expect it to accept. Test-harness / runtime-version compatibility, not a product defect. **Why it still matters:** a developer running `npm test` on current Node sees five failures that do not exist, which erodes trust in the suite. Fix: make the platform tests own their `navigator` state explicitly, or pin/support the intended Node versions. |
