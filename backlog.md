...
| K14 | `api-client.ts` `getAll` treats an unrecognized list shape as success | Medium | OPEN | The `else break` at `api-client.ts:275` exits the page walk when the response is neither an array, `{items}`, nor `{data}` — yielding an empty list that every caller reads as "this tenant has zero entities". A response-shape change would silently blind every ops agent at once. Should throw |
| K15 | Invalid `BACKUP_S3_BUCKET` silently disables backups | Medium | OPEN | `db-maintainer.ts:300-302` returns `{ attempted: false, ok: true }` for a bucket failing the format regex, and `db-maintenance.ts:290` only raises `backup-failed` when `attempted` is true. So a typo'd bucket name produces no backup, no incident and exit 0 — indistinguishable from "backups intentionally off" |
