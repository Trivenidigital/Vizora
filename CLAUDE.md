...
| config-drift-detector | Hourly | **Read-only.** Detects when a *healthy* service can no longer be recreated from its persisted config. Compares effective config vs what a fresh process would consume, and runs each service's own boot validators against the fresh-start config. Also runs one **separate ops-agent credential control** (added 2026-08-15, scope `ops-agents`): resolves `OPS_EMAIL \|\| VALIDATOR_EMAIL` + `OPS_PASSWORD \|\| VALIDATOR_PASSWORD` exactly as the 4 credentialed ops agents do, on each side independently (runtime = PM2 stored env over repo-root `.env`; restart = `.env` alone) and reports HIGH when the persisted config cannot reproduce the pair the agents are authenticating with, CRITICAL when neither path resolves one. Both components are compared as secrets — MATCH/DRIFT tokens only. Never repairs (`issuesFixed` is always 0), never restarts, never writes a config file. Gated by `CONFIG_DRIFT_DETECTOR_ENABLED`, default off |
