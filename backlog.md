...
| B5b | **TrialBanner has the identical swallowed-read defect** | NEW 2026-08-15, IN PR | `.catch(() => {})` + `if (!subscription) return null` (`web/src/components/TrialBanner.tsx:12-16`) hides "Your free trial has ended" from exactly the trial / free-expired orgs it exists to warn. Renders one line below EntitlementBanner in the same container. Same population argument and same degraded-state model as B5 (#350) — apply the fix shipped in #355. **Note on the server sentinel:** `GET /billing/subscription` does have one (`SubscriptionStatus.degraded`), but unlike EntitlementBanner's `status: 'unknown'` it is scoped to the PROVIDER read (`currentPeriodEnd`/`cancelAtPeriodEnd`) and leaves `subscriptionStatus`/`subscriptionTier`/`trialEndsAt` — this banner's only inputs — authoritative, so it must NOT degrade the banner. Pinned by test. |
...
| B5d | **UpgradeBanner is the THIRD notice with the swallowed-read shape** | NEW 2026-08-16, NOT STARTED | Found while fixing B5b. `apiClient.getQuotaUsage().then(setQuota).catch(() => {})` + `if (!quota) return null` (`web/src/components/UpgradeBanner.tsx:16-22`) — an org at or over its screen quota whose read fails sees no approaching-limit or at-limit prompt, which is the population the banner exists for. Mounted in `dashboard/page-client.tsx:466` (the Overview page body, **not** the layout's fixed banner container), so unlike B5/B5b it remounts on every navigation back to Overview — which changes the retry/self-heal calculus and is why it was not folded into B5b. Deliberately NOT fixed in the B5b PR (scope). Same #355 semantic model applies; decide the render policy for a per-navigation mount before copying the pattern verbatim. |
...
| K24 | Sweep the other ops agents for "completeness gates resolution but not action" | Medium | **CLOSED** — both axes discharged across all four agents | K13's root shape was: a completeness verdict was computed, wired to the incident-resolution set, and NOT wired to the destructive write it was computed for. `fleet-manager`, `schedule-doctor` and `db-maintainer` all took coverage keys from the same PR (#353) and all take remediation actions. Audit each for the same asymmetry, and where a write is correctly gated, pin the call site with a source-scan test in the shape of `content-lifecycle-archive-gate.test.ts` — the asymmetry is invisible to behavioural tests on any tenant below the truncation boundary, which is every tenant in production today. **Where the sweep stands after K27:** on the SCAN-completeness question all four agents are now coverage-aware — `content-lifecycle` (K13/#353), `schedule-doctor` (verified clean, not edited: `scanComplete` + `RESOLVABLE_TYPES` at `:63-69 / 134-152 / 444-458`), `db-maintainer` (`lib/db-maintenance.ts:397` already calls `resolveNotReraisedForTypes`), and `fleet-manager` (K27). What remains is the K25 per-ITEM axis, open in exactly one agent: `schedule-doctor` skips an individual entity with no evidence at `:345-346` (an **unknown** playlist item count via `?? -1` is treated as "has items", so `empty_playlist_schedule` clears for a playlist whose emptiness was never observed) and at `:229` (a malformed `endDate` `continue`s, clearing `past_end_schedule` with no verdict). Both want K25's treatment — collect the skipped ids into `currentIncidentIds` — and both must preserve the examined-vs-unexamined distinction K25's test pins. `fleet-manager` was checked on this axis and has no such path (every predicate reads fields already in hand; a failed ping/PATCH is a failed remediation that still re-raises). K24 closes when those two land. **CLOSED — both landed.** Final disposition of the per-ITEM axis, all four agents enumerated: (1) **content-lifecycle** — closed by K25/#361, `confirmReadFailedIds` + `deferredCandidateIds` folded into `currentIncidentIds`; (2) **fleet-manager** — closed by K27/#364 as a *documented no-op*, every predicate reads fields already on the fetched display and the one deliberate drop (`isDisabled`) is evidence-bearing per #259, pinned by `fleet-manager-disabled.test.ts`; (3) **schedule-doctor** — closed here, the two evidence-free skips (`isNaN(endDate)` and an undeterminable playlist item count) now collect their ids; (4) **db-maintainer** — **no per-item surface exists**: `VACUUM_TABLES` is a fixed hardcoded list, not a fetched universe that can truncate, and its single per-item skip (`db-maintainer.ts:181-183`, table missing) *pushes a `VacuumOutcome`* before it `continue`s, so every table produces a recorded verdict every run; its sweep's only coverage inputs (`tableCheckRan`, `backupConfigured`) are per-CHECK and already wired. Residual audited and found clean in schedule-doctor: every remaining `continue` reads a real field value (`isActive`, absent `endDate`/`playlistId`, `displayIds.has`, `playlistMap` miss under a complete scan, `endDate >= now`, `itemCount !== 0`, `currentPlaylistId`, `scheduledDisplayIds.has`). **One unrelated finding surfaced and deliberately NOT fixed here — see K28** (orphan check reads the disabled-filtered display set) |
| K27 | `fleet-manager` resolves stale incidents off a possibly-truncated scan — no completeness gate at all | Medium | **IN PR** — `fix/fleet-manager-coverage-aware-clearing` | The K25/K24 shape, in the agent that has none of the machinery. `main` fetches with `api.getAll<DisplayItem>('/displays')` + `getAll<ScheduleItem>('/schedules')` (`fleet-manager.ts:138-141`), and `getAll` is `getAllScan(...).items` with the `complete` verdict **discarded** (api-client.ts:305-307) — the docblock there says in as many words that callers deciding whether they saw the whole collection must use `getAllScan`. Displays past `MAX_ENTITIES=500` are then never examined, raise nothing, and `resolveNotReraised(priorState.incidents, AGENT, currentIncidentIds)` at `:436` — the NON-coverage-aware variant, with no `scanComplete` gate anywhere in the file (`grep -c 'scanComplete\|getAllScan\|coveredTypes'` = 0) — resolves every prior open/escalated incident they hold: `display_offline`, `display_offline_persistent`, `display_error`, `no_content`, and the per-org `cluster_offline`. The same truncation cuts the other way too: schedules past 500 are unseen, so Check 4 can raise **false** `no_content` for a display that is in fact covered. Fix shape: `getAllScan`, gate the resolution loop on `displayScan.complete && scheduleScan.complete`, and move to `resolveNotReraisedForTypes` with an explicit type set + a `scan-truncated` incident, mirroring `schedule-doctor.ts:63-69 / 198-221 / 444-458`. Latent at prod's fleet size; unlike content-lifecycle this agent's blast radius is the whole tenant's screens. Two weaker adjacent cases in `schedule-doctor` for the same sweep: `:345-346` treats an **unknown** playlist item count (`?? -1`) identically to "has items", so `empty_playlist_schedule` clears for a playlist whose emptiness was never observed; `:229` `continue`s on a malformed `endDate`, clearing `past_end_schedule` with no verdict. **Fixed as the described shape** — `getAllScan` on both lists, `scanComplete = displayScan.complete && scheduleScan.complete` measured on the RAW scans (before the `isDisabled` filter, which only shortens), the sweep moved to `resolveNotReraisedForTypes` over an explicit six-type set, and an info-severity `scan-truncated` that is NOT counted in `issuesFound`. **No K25-shape per-ITEM gap exists here and none was added**: every predicate (`minutesSinceLastSeen`, `isErrorState`, `isOnline`, playlist/schedule membership) reads fields already on the fetched display, so no display reaches a check without a verdict; a failed ping or a failed error-state PATCH is a failed *remediation* that still raises the incident, keeping the id in `currentIncidentIds`. The one deliberate drop, `isDisabled`, is evidence-BEARING and must keep resolving (#259). Also corrected the now-false claim in `state.ts` that `resolveNotReraised` was safe because "fleet-manager recomputes its full set every run" — it recomputed from `getAll`. The two schedule-doctor per-item cases above are NOT fixed here and are what keeps K24 open |
| K28 | schedule-doctor auto-deactivates schedules that target an operator-DISABLED display, calling them orphans | Medium | **READY FOR PR** — `fix/schedule-doctor-existence-oracle` (branch pushed; PR not yet opened, CI is externally blocked) | Found while auditing the per-item skips for K24 and left alone because it is the OPPOSITE direction (a false-positive RAISE with an auto-mutation, not a silent clear) and because auto-deactivating an operator's schedules is customer-visible enough to deserve its own decision. `schedule-doctor.ts:203` reassigns `displays` to the `isDisabled !== true` subset — correct for check 4, which is what #259 was about — but `:236` then builds `displayIds` from that ALREADY-FILTERED list, and check 2 tests `displayIds.has(sched.displayId)` at `:354`. So an active schedule pointing at a display that exists but is merely disabled falls through to the orphan branch: it is PATCHed to `isActive: false` and fires a **critical** §12b alert stating the display "no longer exists" — which is false. Plausible in normal operation: an operator disables a screen and its active schedules remain. Fix is one line (build `displayIds` from the raw scan before the filter, keeping the filter for check 4 only), but it changes remediation behaviour, so it wants its own PR + a test that a schedule targeting a disabled display is neither deactivated nor alerted on **FIXED.** Three changes, in the order they matter: (1) the rebinding is gone — `allDisplays` (the existence universe, straight from the scan) and `alertableDisplays` (the `isDisabled`-filtered list) are separate bindings, `displayIds` is built from the first, and check 4 keeps iterating the second so #259 still holds; (2) check 2 is gated on `scanComplete` — a truncated walk cannot prove nonexistence, which was the WORSE of the two paths: `scanComplete` gated only incident RESOLUTION while the four checks ran unconditionally, so one org crossing the 500-entity cap would have mass-deactivated every active schedule targeting a display beyond it in a single run; (3) the check is now NON-MUTATING and `warning`, not `critical` — the PATCH and the inline alert are gone and the message says "absent from this scan" rather than asserting a deletion the FK cascade makes impossible. The check is KEPT rather than deleted: with the oracle fixed, a surviving fire means THIS AGENT'S VIEW is broken (org scope, response-shape drift, a short page walk), which is a useful invariant monitor — it is just worth nothing to auto-"fix", because mutating customer schedules off a view we have just called unreliable is the whole K28 damage. Prod state at fix time: 0 orphan rows (cascade confirmed), 5 disabled displays all in the ops principal's own org (precondition ARMED), 0 active schedules currently targeting one (so it had never fired — 0 incidents, 0 remediations, 0 log mentions), and `SLACK_WEBHOOK_URL` UNSET, so had it fired the operator would have been told nothing. Tests: 4 spawn-harness cases + a 6-case source-scan file (`schedule-doctor-orphan-gate.test.ts`); the fixture server now RECORDS PATCH bodies, because "no mutation happened" is the load-bearing assertion and an incident-only assertion stays green for a rewrite that keeps the write and drops the finding. |
| K29 | A schedule deactivation writes NO audit row, so agent edits are indistinguishable from operator edits | Low | OPEN — surfaced by K28 | There are **zero** `auditLog.create` calls anywhere under `middleware/src/modules/schedules/`, and `Schedule` carries no `updatedBy` (`schema.prisma` — the model has `createdAt`/`updatedAt` and nothing about actorship). So when `schedule-doctor` PATCHes `isActive: false`, the only trace is `updatedAt` plus the agent's own `ops-state.json` remediation log — nothing in the database says an agent did it. That is exactly why establishing whether the K28 false positive had ever fired on prod needed a heuristic (cross-referencing ops-state remediations and log mentions) rather than a query. It also means an operator who finds a schedule mysteriously off cannot tell an agent action from a colleague's. The residual risk is smaller after K28 — check 2 no longer writes at all, and check 1 (past-end) is a defensible auto-fix — but any FUTURE agent write to schedules should land together with the audit row. Fix shape: an `AuditLog` write in `SchedulesService.update` recording actor + before/after, or an `updatedBy` column; the former is preferable because `AuditLog` already exists and already carries the actor. |
