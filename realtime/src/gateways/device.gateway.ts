  ...
  implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit
{
  ...
  private async sendInitialState(client: Socket, deviceId: string, orgId: string): Promise<void> {
    try {
      ...
      //
      // Layout zones (PD-9) are resolved INSIDE the shared resolver, so layouts are
      // identical on both channels — this replaces the realtime-only
      // resolveLayoutContent that made push != pull for layouts.
      const effective = await resolveEffectiveContent(
        this.databaseService,
        deviceId,
        orgId,
      ...
      const payload = serializeDeviceContent(effective, {
