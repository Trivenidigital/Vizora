...
describe('App', () => {
    ...
    it('renders with initial settings', () => {
        render(_jsx(App, { socket: mockSocket, initialSettings: initialSettings }));
    ...
    it('handles settings updates', async () => {
        ...
        await act(async () => {
            const settingsCallback = mockSocket.on.mock.calls.find((call) => call[0] === 'settings:update')[1];
    ...
    it('handles content updates', async () => {
        render(_jsx(App, { socket: mockSocket, initialSettings: initialSettings }));
        await act(async () => {
            const contentCallback = mockSocket.on.mock.calls.find((call) => call[0] === 'content:update')[1];
    ...
    it('handles schedule updates', async () => {
        render(_jsx(App, { socket: mockSocket, initialSettings: initialSettings }));
        await act(async () => {
            const scheduleCallback = mockSocket.on.mock.calls.find((call) => call[0] === 'schedule:update')[1];
    ...
    it('cleans up event listeners and intervals on unmount', () => {
        jest.useFakeTimers();
        const { unmount } = render(_jsx(App, { socket: mockSocket, initialSettings: initialSettings }));
        unmount();
