...
describe('useRealtimeEvents', () => {
  ...
  describe('Health Alerts', () => {
    it('should handle health:alert events', async () => {
      ...
      act(() => {
        mockSocket?.simulateEvent('health:alert', mockAlert);
      });
    ...
    it('should distinguish alert severities', async () => {
      ...
      act(() => {
        mockSocket?.simulateEvent('health:alert', criticalAlert);
      });

      act(() => {
        mockSocket?.simulateEvent('health:alert', warningAlert);
      });
  ...
  describe('Schedule Execution', () => {
    it('should handle schedule:executed events', async () => {
      ...
      act(() => {
        mockSocket?.simulateEvent('schedule:executed', mockExecution);
      });
    ...
    it('should track schedule execution states', async () => {
      ...
      actions.forEach((action) => {
        act(() => {
          mockSocket?.simulateEvent('schedule:executed', {
            scheduleId,
            displayId: 'device-1',
            playlistId: 'playlist-1',
            action,
