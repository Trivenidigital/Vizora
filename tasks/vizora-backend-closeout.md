...
| 5 | Identity & security | 3 | **3.5 → 3 (REVISED DOWN 2026-07-04)** | `auth/check` = sole authenticated revocation authority (false-410 impossible); B10 token expiry + B16 JWT-TTL hardening landed. **Correction (PD-4): the 3.5 assumed per-route rate limits worked — they DO NOT.** Every `@Throttle({ default })` in the tree is dead metadata (`ThrottlerModule` registers only named `short/medium/long`, never `default`), so intended stricter limits on **auth/login** and other sensitive endpoints silently run at the generous global ceiling. Plus `checkPairingStatus` returns the 90-day device JWT to any unauthenticated caller holding the pairing code. Device-auth + login rate-limiting are both weaker than the audit assumed → honest score is **3**, not 3.5, until the throttle-wiring ticket (PD-4) lands with its own review. |
