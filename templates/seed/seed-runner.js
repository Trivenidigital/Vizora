/**
 * Plain-JS seed runner — works on Windows without ts-node.
 * Uses direct path to the generated Prisma client.
 *
 * Usage:
 *   node templates/seed/seed-runner.js --clear
 *   node templates/seed/seed-runner.js --category=restaurant
 */
...
async function main() {
  ...
  try {
    console.log('Template Library Seeder v2 (JS runner)');
    ...
    let systemOrg = await prisma.organization.findFirst({ where: { name: 'Vizora System' } });
    if (!systemOrg) {
