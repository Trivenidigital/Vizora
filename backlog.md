...
| K28 | schedule-doctor auto-deactivates schedules that target an operator-DISABLED display, calling them orphans | Medium | OPEN — found during K24, deliberately not fixed there | Found while auditing the per-item skips for K24 and left alone because it is the OPPOSITE direction (a false-positive RAISE with an auto-mutation, not a silent clear) and because auto-deactivating an operator's schedules is customer-visible enough to deserve its own decision. `schedule-doctor.ts:203` reassigns `displays` to the `isDisabled !== true` subset — correct for check 4, which is what #259 was about — but `:236` then builds `displayIds` from that ALREADY-FILTERED list, and check 2 tests `displayIds.has(sched.displayId)` at `:354`. So an active schedule pointing at a display that exists but is merely disabled falls through to the orphan branch: it is PATCHed to `isActive: false` and fires a **critical** §12b alert stating the display "no longer exists" — which is false. Plausible in normal operation: an operator disables a screen and its active schedules remain. Fix is one line (build `displayIds` from the raw scan before the filter, keeping the filter for check 4 only), but it changes remediation behaviour, so it wants its own PR + a test that a schedule targeting a disabled display is neither deactivated nor alerted on |
| K29 | `emitWithDeliveryAck` FABRICATES `delivered: true` for non-ack-capable clients — and two consumers act on it | **High** | OPEN — settle BEFORE any "Delivered" UI | `device.gateway.ts:276-279` returns `{ delivered: true, legacy: true }` for any client that does not advertise the `deliveryAck` capability, **without observing anything**. Nothing user-visible rests on it today (the Playlists redesign, #363, deliberately states only `assigned`), but two consumers do not discriminate on `legacy` where it matters: `deliverPendingCommands` (:2142) **dequeues on `delivered`**, so a command lost in a half-open socket is dropped permanently with no retry and no operator signal; `replayPendingPlaylist` (:2087) calls `recordSentContentVersion` on the fabricated success, writing a possibly-false "device holds version X" that then **suppresses the reconcile that would have repaired it**. Fix shape: (1) rename so `sent` means sent and `delivered` means acked — the current name is what invites the two misuses; (2) make the legacy path an **explicit policy** rather than an implicit optimism: retain-and-retry with idempotency keys, or drop-with-alert (§12b), chosen deliberately. **Sizing input we do not have:** the Electron client declares `deliveryAck: true`, but the Android TV client lives in the separate `vizora-tv` repo — **if it is not ack-capable, every command to the primary fleet takes the fabricated path**, which moves this from latent to live. Check that first. **This must be settled before anyone builds a "Delivered"/"Playing" badge**, because this value is precisely what would turn such a badge into a lie — see the `assigned != delivered != acknowledged != playing` invariant in CLAUDE.md |
