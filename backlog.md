...
| B3b | **`total_count: 12` silently ends every monthly Razorpay subscription after a year** | NEW 2026-08-15, NOT STARTED | `razorpay.provider.ts:110` creates every subscription with `total_count: 12`. For a MONTHLY plan that means billing stops after twelve charges and Razorpay emits `subscription.completed` — the customer is silently off-subscription while still expecting service. B3 routes that event so entitlement now collapses to free instead of being retained for free forever, which fixes the *entitlement* half. The remaining question is **commercial and not answerable from code**: should a monthly subscription auto-renew indefinitely (`total_count` high or omitted), should the customer be prompted to re-subscribe at twelve months, or is a one-year commitment the intended product? Whatever is chosen also has to be right for the yearly plans, where `total_count: 12` means twelve YEARS. Needs a product decision before any code change. |
