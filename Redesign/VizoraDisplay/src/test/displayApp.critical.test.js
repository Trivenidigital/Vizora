...
describe('DisplayApp', () => {
    ...
    beforeEach(() => {
        ...
        DeviceAuthService.mockImplementation(() => mockDeviceAuthService);
        VizoraSocketClient.mockImplementation(() => mockSocketClient);
    });
    afterEach(() => {
        vi.clearAllMocks();
    });
    it('shows loading screen initially', () => {
        render(_jsx(DisplayApp, {}));
    ...
    it('handles successful registration', async () => {
        ...
        await waitFor(() => {
            expect(mockDeviceAuthService.registerWithPairingCode).toHaveBeenCalledWith('test-code', expect.any(Object));
        });
    });
    it('handles registration failure', async () => {
        mockDeviceAuthService.getToken.mockReturnValue(null);
        mockDeviceAuthService.getDisplayId.mockReturnValue(null);
        mockDeviceAuthService.registerWithPairingCode.mockRejectedValue(new Error('Registration failed'));
        render(_jsx(DisplayApp, {}));
        // Wait for registration screen
        await waitFor(() => {
            expect(screen.getByText(/display registration/i)).toBeInTheDocument();
