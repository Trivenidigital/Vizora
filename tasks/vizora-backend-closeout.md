...
|---|---|---|---|---|
| 1 | Tenant isolation & content integrity | 3 | **4** (4→5 gated on `docs/enforce-readiness.md`) | Org-scoped in-statement writes across the CRUD services; B12 proves isolation e2e. The tenant-guard is **built + safe but log-only (never blocks)**. **The 4→5 completes only when enforce-mode ships** — and the honest status is that enforce was gated on a prerequisite set that was still being *discovered*, not just executed: three blocks of prep kept flushing out inherited-context write surfaces (webhook bypass, then the @OnEvent handlers). **Enforce surface fully executed keyboard-side** (all checked on their own evidence, no third category): CRUD sweep, system-write bypass, @OnEvent, device-auth, realtime, nested creates — #1–#6 all ✅. What remains is exactly TWO structural items, both gated to the operator: (1) the flip itself (`TENANT_GUARD_MODE=enforce`, after a log-warn review) and (2) the realtime `$use` guard (#7 — makes enforcement structural in realtime, not just conventional). **dimension-1 stays 4 — worded "structural in middleware, convention-safe in realtime" — until BOTH land** (asymmetric enforcement is not a structural 5). Single source of truth: `docs/enforce-readiness.md` incl. the flip-readiness statement. A legitimate, earned 4; 5 is two operator-gated items away. |
...
| 8 | Testing | 3 | **4** | Strong negative suites across contract/billing/entitlement/tenant-iso, plus the B12 two-tenant lifecycle E2E (10/10 green) asserting the revocation contract + zero cross-tenant bleed at integration level. Not 5: no cross-service socket-handshake E2E (realtime is a separate app; covered by realtime unit tests). **STANDING OBSERVATION (for re-score): unit-green is necessary but NOT sufficient in this repo — the full typecheck / cross-app build is load-bearing, because the unit mocks hide integration seams.** Evidence: FIVE build-verify catches across the T2 increments where isolated tests were green but the app build was not — (1) jest wholesale-mocks `@vizora/database` so moved pure helpers needed re-export; (2) ts-jest CommonJS vs tsc-ESM `.js` resolution; (3) `verifyCurrentDeviceToken` needs `databaseService` (a unit test mocked past it); (4) removing a method orphaned two imports; (5) the resolver `db` Pick type missing `content`. Structural gap in the mock strategy (tests mock the boundary they should exercise) — the same mock-hides-integration property that makes B12's live-DB run + offshore's hardware sitting load-bearing rather than ceremonial. Fix is integration/typecheck gates in CI, not more unit mocks. |
