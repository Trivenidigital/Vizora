...
|---|---|---|---|---|
| 1 | Tenant isolation & content integrity | 3 | **4** (4→5 gated on `docs/enforce-readiness.md`) | Org-scoped in-statement writes across the CRUD services; B12 proves isolation e2e. The tenant-guard is **built + safe but log-only (never blocks)**. **The 4→5 completes only when enforce-mode ships** — and the honest status is that enforce was gated on a prerequisite set that was still being *discovered*, not just executed: three blocks of prep kept flushing out inherited-context write surfaces (webhook bypass, then the @OnEvent handlers). Two blocks **confirmed enumeration**: CRUD sweep DONE, system-write bypass DONE, @OnEvent CHECKED-CLEAN, and the realtime trust boundary (the one item that could still have surfaced a new write class) CHECKED-CLEAN — safe-by-construction, **no third category**. The enforce surface is now enumerated, not still-discovering. Remaining is MECHANICAL: device-auth write paths + nested creates + a log-warn review. Single source of truth: `docs/enforce-readiness.md`. A legitimate 4; 5 is genuinely close — lands when those two mechanical items clear. |
