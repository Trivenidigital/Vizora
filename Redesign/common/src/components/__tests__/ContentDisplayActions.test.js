...
describe('ContentDisplayActions', () => {
    const mockDisplays = [
        { id: '1', name: 'Display 1', status: 'online' },
        { id: '2', name: 'Display 2', status: 'offline' },
    ];
    ...
    it('renders display selection interface', async () => {
        render(_jsx(ContentDisplayActions, { displays: mockDisplays, onPush: mockOnPush, onSchedule: mockOnSchedule, onCancel: mockOnCancel }));
        await waitFor(() => {
    ...
    it('checks for schedule conflicts', async () => {
        ...
        expect(mockOnSchedule).toHaveBeenCalledWith(['1'], expect.objectContaining({
